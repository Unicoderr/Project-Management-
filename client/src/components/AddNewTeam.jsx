...
//     // Remove the empty link objects from TeamTools
...
//     try {
//       const res = await createTeam(Team, token);
//       // get the id from res and invite members
//       await handleInviteAll(res.data._id);
//       setLoading(false);
//       setNewTeam(false);
//       dispatch(
//         openSnackbar({
...
import ImageSelector from "./ImageSelector";
import { tools } from "../data/data";
import { createTeam, searchUsers, inviteTeamMembers } from "../api/index";
...
const AddNewTeam = ({ setNewTeam }) => {
  ...
  const [inputs, setInputs] = useState({ img: "", name: "", desc: "", tags: [] });
  const [TeamTools, setTeamTools] = useState([
    { name: "", icon: "", link: "" },
    { name: "", icon: "", link: "" },
    { name: "", icon: "", link: "" },
  ...
  useEffect(() => {
    setDisabled(inputs.name === "" || inputs.desc === "");
  }, [inputs]);
  ...
  const handleSearch = async (e) => {
    setSearch(e.target.value);
    try {
      const res = await searchUsers(e.target.value, token);
      setUsers(res.data);
    } catch (err) {
      setUsers([]);
    }
  ...
  const handleInviteAll = async (id) => {
    for (const user of selectedUsers) {
      try {
        await inviteTeamMembers(id, user, token);
        dispatch(
          openSnackbar({
            message: `Invitation sent to ${user.name}`,
            type: "success",
        ...
        dispatch(
          openSnackbar({
            message: `Invitation can't be sent to ${user.name}`,
            type: "error",
          })
        );
      }
  ...
  const handleCreateTeam = async () => {
    ...
    // Remove duplicate projects from the list
    const uniqueProjects = Array.from(new Set(TeamTools.map(tool => tool.link)));
  
    const teamData = {
      name: inputs.name,
      description: inputs.desc,
      tags: inputs.tags,
      tools: uniqueProjects.map(link => {
        const tool = TeamTools.find(tool => tool.link === link);
        return { name: tool.name, icon: tool.icon, link };
      }),
    ...
    try {
      ...
      await handleInviteAll(newTeam._id);
      dispatch(
        openSnackbar({
          message: "Team created successfully",
          type: "success",
            ...
            <>
              ...
              <ButtonContainer>
                ...
                <OutlinedBox
                  ...
                  activeButton={!disabled}
                  style={{ marginTop: "18px", width: "100%" }}
                  onClick={() => { !disabled && handleCreateTeam() }}
                >
                  {Loading ? (
                    <CircularProgress color="inherit" size={20} />
                  ) : (
