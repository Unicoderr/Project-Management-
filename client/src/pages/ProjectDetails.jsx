...
const ToggleButton = styled.div`
  cursor: pointer;
  background-color: ${({ alignment, button }) => (alignment && button === "col") || (!alignment && button === "row") ? "#1976D2" : "transparent"};
  color: ${({ alignment, button }) => (alignment && button === "col") || (!alignment && button === "row") ? "#fff" : "#1976D2"};
  padding: 5px 10px;
  border-radius: 4px;
  display: flex;
  align-items: center;
...
const Column = styled.div`
  display: flex;
  flex-direction: ${({ alignment }) => (alignment ? "column" : "row")};
  gap: 20px;
`;
...
const ProjectDetails = () => {
  ...
  const handleAddNewWork = () => {
    ...
    // Assuming an API call to add the new work
  ...
  const updateWorkStatus = (workId, newStatus) => {
    setWorks(works.map(work => work.id === workId ? { ...work, status: newStatus } : work));
  };
  ...
  return (
    <Container>
      ...
      {loading ? (
        ...
        <>
          <Header>
            <AddNewButton onClick={() => setAddDialogOpen(true)}>
              <AddIcon />
              Add New Work
            </AddNewButton>
          </Header>
            ...
            <Work>
              ...
              <Column alignment={alignment}>
                <ItemWrapper>
                  <Top>
                    <Text>
                      ...
                      <Span>
                        ({" "}
                        {works.filter((item) => item.status === "Working").length}{" "}
                        )
                      </Span>
                    </Text>
                  </Top>
                  <ResponsiveMasonry columnsCountBreakPoints={{ 350: 1, 750: 2, 900: 2 }}>
                    <Masonry gutter="14px">
                      {works.filter((item) => item.status === "Working").map((item) => (
                        <div onClick={() => openWorkDetails(item)}>
                ...
                <ItemWrapper>
                  <Top>
                    <Text>
                      <CheckCircleOutlineOutlined sx={{ color: "#67BC6D", fontSize: "20px" }} />
                      Completed
                      <Span>
                        ({" "}
                        {works.filter((item) => item.status === "Completed").length}{" "}
                        )
                      </Span>
                    </Text>
                  </Top>
                  <ResponsiveMasonry columnsCountBreakPoints={{ 350: 1, 750: 2, 900: 2 }}>
                    <Masonry gutter="14px">
                      {works.filter((item) => item.status === "Completed").map((item) => (
                        <div onClick={() => openWorkDetails(item)}>
            ...
            <Extra>
              ...
              <Table>
                ...
                {tasks.map((task, index) => (
                  <TaskCard key={task.id} index={index + 1} task={task} />
                ))}
              </Table>
            </Extra>
          </Body>
          ...
          <Dialog open={addDialogOpen} onClose={() => setAddDialogOpen(false)}>
            ...
            <DialogActions>
              ...
              <Button onClick={handleAddNewWork} color="primary">
                Add
              </Button>
            </DialogActions>
          </Dialog>
