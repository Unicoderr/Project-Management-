...
//     try {
//         const { name, description, tags, tools, members, img } = req.body;
...
//         await User.findByIdAndUpdate(user.id, { $push: { teams: savedTeams._id } }, { new: true });
...
export const createTeam = async (req, res) => {
    try {
        const { name, description, tags, tools, members, img, projects } = req.body;

        if (!name || !description) {
            return res.status(400).json({ message: 'Name and description are required' });
        }

        // Check if projects field is provided and is unique
        if (projects && new Set(projects).size !== projects.length) {
            return res.status(400).json({ message: 'Duplicate projects are not allowed' });
        }
        ...
        if (error.code === 11000 && error.keyPattern && error.keyPattern.projects) {
            return res.status(400).json({ message: 'Duplicate projects are not allowed' });
        }
...
export const addTeam = async (req, res, next) => {
    ...
    const newTeams = new Team({ members: [{ id: user.id, role: "d", access: "Owner" }], ...req.body });
    try {
...
export const getTeam = async (req, res, next) => {
    try {
        const team = await Team.findById(req.params.id).populate("members.id", "_id  name email img").populate({
            path: "projects",
            populate: {
                path: "members.id",
                select: "_id name email",
...
export const updateTeam = async (req, res, next) => {
    try {
        ...
        return next(createError(403, "You can update only if you are a member of this Team!"));
...
export const addTeamProject = async (req, res, next) => {
    ...
    try {
        ...
        Team.findByIdAndUpdate(req.params.id, { $push: { projects: saveProject._id } }, { new: true }, (err, doc) => {
            if (err) {
                next(err);
            }
        });
    ...
    try {
        ...
        if (code === req.app.locals.CODE) {
            ...
            await Team.findByIdAndUpdate(
                ...
                //add team id and team name to user
                await User.findByIdAndUpdate(
                    userid,
                    {
                        $push: { teams: team.id },
